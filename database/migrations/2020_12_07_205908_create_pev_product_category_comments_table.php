...
class CreatePevProductCategoryCommentsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('pev_product_category_comments', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('type', ['question', 'comment'])->default('comment');
            $table->float('grade', 12, 2);
            $table->string('image', 128);
            $table->boolean('active')->default(1)->unsigned();
            $table->string('ip', 255);//ip del usuario.
    ...
    {
        Schema::dropIfExists('pev_product_category_comments');
    }
}
