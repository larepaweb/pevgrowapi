...
class PevRedirectionsSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        DB::table('pev_redirections')->delete();
        DB::statement('ALTER TABLE pev_redirections AUTO_INCREMENT=1;');
        DB::statement('SET FOREIGN_KEY_CHECKS=1;');
        // $file = File::get("database/data/pev_cms.json");
        // $file = json_decode($file);
        $file = fopen("database/data/pev_redirections.json", "r");//aperturo el archiv .json desde el path donde se encuentra.
        $cont ="";
            while(!feof($file)) {
        ...
        for ($i=0; $i < $cantidad; $i++) { 
            ...
            DB::table('pev_redirections')->insert([
                'id' => $cont[$i]->id,
                'url_old' => $cont[$i]->url_old,
                'url_new' => $cont[$i]->url_new,
                'redirect_type' => $cont[$i]->redirect_type,
