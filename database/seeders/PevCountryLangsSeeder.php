...
class PevCountryLangsSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        DB::table('pev_country_langs')->delete();
        DB::statement('ALTER TABLE pev_country_langs AUTO_INCREMENT=1;');
        DB::statement('SET FOREIGN_KEY_CHECKS=1;');
        // $file = File::get("database/data/pev_cms.json");
        // $file = json_decode($file);
        $file = fopen("database/data/pev_country_langs.json", "r");//aperturo el archiv .json desde el path donde se encuentra.
        $cont ="";
            while(!feof($file)) {
        ...
        for ($i=0; $i < $cantidad; $i++) { 
            ...
            DB::table('pev_country_langs')->insert([
                'id' => $cont[$i]->id,
                'pev_country_id' => $cont[$i]->pev_country_id,
