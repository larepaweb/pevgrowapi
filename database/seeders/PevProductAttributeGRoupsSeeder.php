...
class PevProductAttributeGRoupsSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        DB::table('pev_product_attribute_groups')->delete();
        DB::statement('ALTER TABLE pev_product_attribute_groups AUTO_INCREMENT=1;');
        DB::statement('SET FOREIGN_KEY_CHECKS=1;');
        // $file = File::get("database/data/pev_cms.json");
        // $file = json_decode($file);
        $file = fopen("database/data/pev_product_attribute_groups.json", "r");//aperturo el archiv .json desde el path donde se encuentra.
        $cont ="";
            while(!feof($file)) {
        ...
        for ($i=0; $i < $cantidad; $i++) { 
            ...
            DB::table('pev_product_attribute_groups')->insert([
                'id' => $cont[$i]->id,
                'is_color_group' => $cont[$i]->is_color_group,
                'group_type' => $cont[$i]->group_type,
